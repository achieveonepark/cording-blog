---
title: "Unity Build Play Tool"
description: "AI로 만든 빌드 실행 툴!"
pubDate: 2026-03-24
tags:
  - Unity
  - Build
category: R&D
thumbnail: /images/1774359786310.png
---

# 뭔데요...?
유니티에서 빌드를 뽑았을 때 빌드를 환경에 맞는 디바이스에 옮기는 게 생각보다 정말 귀찮다. 
딸깍 한번으로 빌드를 설치 및 실행하기 위해 만든 툴.
Claude와 Codex를 섞어 만들었는데 생각보다 잘 뽑아준다... 우와...

# 간략하게 보기

![image](/images/1774359786310.png)

- Android: 기기 연결 시 Refresh 버튼 누르면 뜸, 빌드 선택하여 설치
- iOS: xCode 플젝 실행
- WebGL: localhost:port로 서버 실행 후 테스트

# 다운로드

## [Mac](https://github.com/achieveonepark/unity-build-tool/blob/main/artifacts/unity-build-tool-0.1.0-macos-arm64.dmg)

## [Windows](https://github.com/achieveonepark/unity-build-tool/blob/main/artifacts/unity-build-tool-0.1.0-windows-x64-setup.exe)